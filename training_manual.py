...
def train(lr, epochs, model, stage, params):
    ...
    best_model = models.load_model("./results/manual/aux.keras")
...
def show_graph(name, history, labels, predictions):
    ...
    path = "./results/manual/aux.png"
    plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1, hspace=0.4, wspace=0.4)
    plt.savefig(path)
    plt.show()

def clean(name):
    path = "./results/manual/"+name+".png"
    if os.path.exists(path):
        os.remove(path)
    
    path = "./results/manual/"+name+".keras"
    if os.path.exists(path):
        os.remove(path)
...
def save_model(name):
    shutil.copy(
        "./results/manual/aux.keras",
        "./results/manual/"+name+".keras"
    )
    shutil.copy(
        "./results/manual/aux.png",
        "./results/manual/"+name+".png"
    )
...
# accuracy: 0.9030 - loss: 0.4138 - val_accuracy: 0.6903 - val_loss: 0.7819
# Test accuracy: 0.6690140962600708 - Test loss: 1.0059480667114258
...
# accuracy: 0.9596 - loss: 0.2401 - val_accuracy: 0.6814 - val_loss: 0.8257
# Test accuracy: 0.7112675905227661 - Test loss: 0.8051881194114685
get_best(parameters,3,0,0.00002,15,"3-0-2")

# learning rate = 0.00002, epochs = 10-15, val acc ~ 75
parameters={"op":"sgd","lo":"sparse_categorical_crossentropy","nod":2,"nn":512,"dim":(224,224)}
# accuracy: 0.9323 - loss: 0.3433 - val_accuracy: 0.6903 - val_loss: 0.7502
# Test accuracy: 0.7112675905227661 - Test loss: 0.7633513808250427 
...
# accuracy: 0.9828 - loss: 0.1036 - val_accuracy: 0.6106 - val_loss: 1.5196
# Test accuracy: 0.6760563254356384 - Test loss: 1.168778419494629
...
# accuracy: 1.0000 - loss: 0.0191 - val_accuracy: 0.6903 - val_loss: 1.4717
# Test accuracy: 0.6690140962600708 - Test loss: 1.739703893661499
