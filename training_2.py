...
def train_model_3(model, images_train, labels_train, images_val, labels_val, f, dim):
    return model.fit(greyscale_to_rgb(images_train[f],dim),np.array(labels_train[f]),shuffle=True,epochs=EPOCHS,validation_data=(greyscale_to_rgb(images_val[f],dim), np.array(labels_val[f])))
...
backtracking(0,parameters,parameters_values,chosen,images,labels,create_model_3,train_model_3,"./models/model_3/results_1.csv")
